...
- **scripts/upload-oss-artifact.sh** — 统一 OSS 上传脚本，自动规范化 `OSS_REGION` 格式（兼容 `cn-hangzhou` 和 `oss-cn-hangzhou`），安装 ossutil、**gzip 压缩 APK** 后上传为 `.bin.gz`（绕过 OSS 对 APK 文件头的 `ApkDownloadForbidden` 拦截）、生成并上传 `version.json`
