...
- **android-build.yml** — push 到 main/master 或 PR 时构建调试 APK（始终触发，用于保持 CI 缓存新鲜）
- **android-publish.yml** — 仅当 `app/`、`build.gradle`、`settings.gradle`、`gradle.properties`、`gradle/**`、`gradlew`、`gradlew.bat` 变更时触发，发布 GitHub prerelease，tag 为 `v{VERSION_NAME}-{SHA12}`，幂等
