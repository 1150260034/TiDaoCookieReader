...
if ! grep -Eq '<external-files-path[^>]*\bname[[:space:]]*=[[:space:]]*"[^"]*"[^>]*\bpath[[:space:]]*=[[:space:]]*"[^"]*"' <<< "$content" \
  && ! grep -Eq '<external-files-path[^>]*\bpath[[:space:]]*=[[:space:]]*"[^"]*"[^>]*\bname[[:space:]]*=[[:space:]]*"[^"]*"' <<< "$content"; then
  echo "$FILE 中的 external-files-path 必须声明 name 和 path 属性"
  exit 1
fi

if grep -Eq '<external-files-path[^>]*\bandroid:(name|path)[[:space:]]*=' <<< "$content"; then
